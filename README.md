...
TuSimple sequences consist of forward-facing camera video clips (1280 × 720 resolution) recorded under predominantly dry, clear-weather daytime highway driving conditions.
...
- The 2,782 evaluation frames were evaluated frame-by-frame; they are not treated as independent IID samples or continuous video streams in this benchmark.
...
1. **HLS Color Space Filtering**:
   ...
   - White markings: `H ∈ [0, 255]`, `L ∈ [185, 255]`, `S ∈ [0, 255]`.
   - Yellow markings: `H ∈ [10, 40]`, `L ∈ [60, 255]`, `S ∈ [60, 255]`.
...
5. **Slope Partitioning and Outlier Filtering**:
   - Calculates segment slope as `m = Δx / Δy`. Segments with `|Δy| < 15 px` or `|m| < 0.25` or `|m| > 2.5` are rejected.
   - Segments are partitioned into left candidates (`x_mid < 0.52 · W`, `m < 0`) and right candidates (`x_mid > 0.48 · W`, `m > 0`).
   - The median slope `m_med` is computed per candidate pool; segments deviating by `|m - m_med| ≥ 0.35` are discarded.
6. **First-Order Linear Fitting**:
   ...
   - If one boundary is missing, `complete_single_lane` synthesizes the opposing line using assumed highway perspective dimensions (`850 px` lane width at the bottom, `280 px` at the horizon).
...
- **Architecture**: Anchor-free detection backbone with C2f feature aggregation, decoupled detection/segmentation head, and non-maximum suppression (85 fused layers, 3,258,454 parameters, 11.3 GFLOPs at `640 × 640`).
- **Classes**: Class 0 (`lane_line` ribbons) and Class 1 (`drivable_area` derived corridor polygon).
- **Post-Processing**: Output masks resized to `1280 × 720`. Boundary coordinates are sampled at near-bumper row `y = 680 px` with a `±5 px` window. If one boundary is missing, the fixed `±420 px` fallback heuristic is applied.
...
- **Architecture**: Lightweight segmentation model evaluated using the Ultralytics implementation in this study (incorporating C3k2 feature blocks, C2PSA attention, and Segment26 head; 136 fused layers / 309 unfused layers, 2,689,274 fused parameters, 9.1 GFLOPs at `640 × 640`).
- **Computational Footprint**: In this configuration, YOLO26n-seg uses approximately 17.5% fewer parameters (569,180 fewer weights) and 19.5% lower reported FLOPs (2.2 GFLOPs lower at `640 × 640`) compared to YOLOv8n-seg.
- **Standardized Evaluation Logic**: Evaluated using the exact same evaluation manifest (`results/evaluation_manifest.csv`), scanline sampling (`y = 680 px`), single-boundary fallback (`±420 px`), and LDWS thresholding (`±80 px`).
...
> [!IMPORTANT]
> **Pipeline Dependency of Lane-Center Error**: Lane-center error reflects the complete lane-position estimation pipeline, including boundary selection, near-bumper scanline extraction, single-boundary fallback, and the fixed `±420 px` assumption; it should therefore not be interpreted as a pure segmentation quality metric. When only one boundary is detected, the static `±420 px` assumption can introduce significant geometric error on curves or roads with non-standard lane widths.
...
1. **Narrow Ribbon IoU Penalty**: Road markings are narrow ribbons (`±6 px` width). A spatial displacement of only 2–3 pixels heavily penalizes Intersection over Union (IoU).
...
Both evaluations executed under identical conditions using scanline `y = 680 px`, confidence threshold `0.25`, and NVIDIA Tesla T4 execution with CUDA synchronization.
...
*Figure 4: Comparative evaluation across 2,782 evaluation frames from the official TuSimple test set. (Top-Left) Spatial pixel error distributions for boundary and center estimates at near-bumper scanline `y = 680 px`. (Top-Right) Per-frame Mean Absolute Error distribution across evaluated frames. (Bottom-Left) Synchronized CUDA runtime latency distributions on NVIDIA Tesla T4. (Bottom-Right) Detection failure rate comparison.*
...
| **Spatial Evaluation Scanline** | `y = 680 px` (Near Bumper) | `y = 680 px` (Near Bumper) | Evaluated over vertical window `y ∈ [675, 685]` |
...
1. **Boundary Precision Comparison (Panel 1: Top-Left)**:
   - When lane markings were detected above the confidence threshold, both architectures located physical lane boundaries with comparable spatial accuracy: YOLOv8n-seg achieved a boundary MAE of `21.42 px` (median `21.00 px`), and YOLO26n-seg achieved `22.44 px` (median `22.50 px`).
   - The boundary RMSE values (`23.39 px` vs `23.59 px`) were separated by only `0.20 px`, indicating similar dispersion when lane masks were successfully extracted.
...
3. **Observed Training Trajectory and Convergence Hypothesis (Panel 4: Bottom-Right)**:
   ...
   - The YOLO26 training losses were still decreasing at epoch 15, indicating that the fixed 15-epoch budget may not have been sufficient for full convergence. The effect of a longer training schedule was not evaluated, so the experiment cannot determine how much of the observed difference is attributable to architecture, training budget, or their interaction.
...
4. **Runtime and Computational Efficiency (Panel 3: Bottom-Left)**:
   ...
   - Mean latency was `10.95 ms` (91.4 FPS) for YOLOv8n-seg and `14.40 ms` (69.5 FPS) for YOLO26n-seg. Both models exceeded 60 FPS under the reported NVIDIA Tesla T4 benchmark configuration.
...
To evaluate the computational cost of edge extraction, Hough accumulator voting, slope partitioning, and linear fitting, the deterministic OpenCV pipeline was profiled across 200 iterations on representative TuSimple highway frames (`1280 × 720`):
...
> **Comparative Timing Context**: Runtime measurements characterize each pipeline under its reported execution hardware and timing methodology; the CPU and GPU measurements should not be interpreted as a direct hardware-normalized comparison. Classical CV executed on the host CPU, where latency varied by over 2.7× (25.7 ms to 70.7 ms) depending on edge density and texture clutter. Learned neural networks executed on GPU tensor cores with relatively stable measured inference latency across the evaluated frames under the fixed inference configuration. Because per-sample timing arrays for these offline runs were not persisted, the summary row reports the unweighted mean latency across the 3 profiled scenes (`42.74 ms`, corresponding to `23.4 FPS`); pooled median and pooled standard deviation are omitted to avoid ungrounded approximations.

#### Historical Synthetic Timing

An earlier benchmark recorded on an empty synthetic frame (`1280 × 720` black array) bypassed portions of the classical pipeline when no line candidates were present, yielding `10.60 ms` mean latency (`10.55 ms` median, `±0.45 ms` std, `94.3 FPS`) on the host CPU. Because an empty frame produces zero Canny edges and bypasses Hough transform accumulator voting and line fitting, this measurement is retained only for historical reproducibility and is not representative of a realistic classical-CV workload.
...
The six-scene quantitative comparison below covers the Classical CV and YOLOv8 pipelines, matching the original per-scene evaluation workflow. YOLO26 qualitative outputs are shown separately in Figure 6. Both pipelines were evaluated under the shared departure warning threshold (`±80.0 px`):

| Scene | Geometry / Challenge | Classical CV Offset | Classical LDWS (±80 px) | YOLOv8 Offset | YOLOv8 LDWS (±80 px) | Comparative Qualitative Finding |
| :--- | :--- | :---:| :---:| :---:| :---:| :--- |
...
> 
> Furthermore, the `±80 px` threshold is a project-specific pixel-space heuristic. It depends on camera mounting height, pitch, lens field-of-view, and image resolution (`1280 × 720`). It is not an automotive safety standard (such as ISO 17361, which evaluates metric distance and time-to-line-crossing) and carries no safety certification.
...
1. **Curvature Underestimation from Linear Fitting**: First-order linear fitting (`x = my + b`) assumes zero road curvature. On curving highway segments (Scenes 1, 2, 6), slope averaging across the trapezoid (`y ∈ [360, 720]`) underestimates lateral vehicle displacement at the bumper.
2. **Static Extrapolation in Single-Lane Recovery**: Both the classical `complete_single_lane` routine and the learned fallback heuristic rely on static pixel offsets (`420 px` at bumper). If lane width deviates from this assumption or if vehicle pitch changes, the synthesized center introduces systematic lateral error.
...
- YOLOv8n-seg achieved lower measured lane-center MAE (`37.62 px` vs `70.06 px`) and median error (`10.50 px` vs `87.50 px`).
- YOLOv8n-seg recovered both ego-lane boundaries on more frames (48.6% vs 16.4%).
- YOLO26n-seg operated with a smaller parameter footprint (`2.69M` vs `3.26M`) and lower computational complexity (`9.1` vs `11.3 GFLOPs`).
...
The final lane-center estimate is not an isolated neural-network output. When only one boundary is detected, the pipeline relies on a fixed `±420 px` geometric recovery heuristic. Consequently, the reported lane-center error reflects the combined performance of:
...
- YOLOv8n-seg: `10.95 ms` mean latency (`91.4 FPS`)
- YOLO26n-seg: `14.40 ms` mean latency (`69.5 FPS`)

These measurements reflect a specific server GPU, input resolution (`640 × 640`), batch size (`1`), and synchronization protocol. While both models demonstrated high throughput in this GPU benchmark, these numbers do not represent automotive embedded SoC deployments (e.g., TI TDA4, Ambarella, or Renesas).
...
- **Sensitivity to Post-Processing Heuristics**: Implementing missing-boundary recovery revealed how a simple geometric assumption (such as a fixed `±420 px` offset) can dominate downstream error, demonstrating that system error reflects the full pipeline rather than raw network quality alone.
...
- **Ground-Truth Spatial Error Evaluation**: Measures boundary-level and lane-center Mean Absolute Error (MAE), Root Mean Square Error (RMSE), and median absolute error at near-bumper scanline `y = 680 px`.
